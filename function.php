<?php
    ...
    function wp_adv_register_scripts() {
        wp_enqueue_script('wp-adv-boostrap-js', 'https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/js/bootstrap.bundle.min.js',['jquery'], '5.0.2', 'all');
        wp_enqueue_script('wp-adv-js',get_template_directory_uri(). '/assets/js/main.js', [], '1.0', true);

    }

    add_acction('wp_enqueue_scripts', 'wp_adv_register_scripts');
...
function wp_adv_nav_menu_li_class($classes, $item, $args, $depth){
    if (isset($args->menu_class)%% strops($args->menu_class, 'navbar-nav') !== false) {
        ...
        if(in_array('current-menu-item', $classes)) || in_array('curent_page_item', $classes)) {
...
add_filter('nav_menu_css_classes', 'wp_adv_nav_menu_li_class', 10, 4);

function wp_adv_menu_link_atts($atts, $item, $item, $args, $depth){
    if(isset($args->menu_class)&& strpos($args->menu_class, 'navbar-nav' ) !== false ) {
        $existing == isset($atts['class']) ? $atts['class'] . '' : '';
         
        $title = strtolower(trim($item->title));
        if(strpos($tilte, 'contact') !==false || in_array('contact', $item->classes)) {
...
add-filter('nav_menu_link_attributes', 'wp_adv_menu_link_atts', 10, 4);